...
def log():
    ...
    print('n_epochs:%d, n_updates:%d, tr_cost:%.3f, va_cost:%.3f, tr_acc:%.2f, va_acc:%.2f'%(n_epochs, n_updates, tr_cost, va_cost, tr_acc, va_acc))
    if submit:
        score = va_acc
        if score > best_score:
            best_score = score
...
if __name__ == '__main__':
    ...
    for i in range(n_iter):
        shuf_data = tuple(shuffle(trX, trM, trYt, random_state=np.random))
        for xmb, mmb, ymb in iter_data(n_batch_train, True, True, float("inf"), *shuf_data):
    ...
    if submit:
        ...
        print('Val accuracy:{}, Test accuracy:{}'.format(val_accuracy, test_accuracy))
