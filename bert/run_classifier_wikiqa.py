...
def eval_map_mrr(qids, aids, preds, labels):
    ...
    for q_id in dic:
        ...
        AP /= float(correct)
        MAP += AP
    tf.logging.info('Overlap: {}/{}'.format(useful_q, len(dic)))
    MAP /= useful_q_len
    MRR /= useful_q_len
    return MAP, MRR
...
class WikiQAProcessor(DataProcessor):
  """Processor for the WikiQA."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "WikiQA-train.txt")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "WikiQA-dev.txt")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "WikiQA-test.txt")), "test")
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    qids, aids, probs, labels = [], [], [], []
    for i, (predict_example, pred) in enumerate(zip(predict_examples, result)):
      q = predict_example.text_a
      a = predict_example.text_b
      prob = float(pred['probabilities'][1])
    ...
    map_score, mrr_score = eval_map_mrr(qids, aids, probs, labels)
    tf.logging.info('MAP:{}, MRR:{}'.format(map_score, mrr_score))
